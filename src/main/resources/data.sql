...
--
-- createUserLink("john", new CreateLinkRequestDto("https://www.baeldung.com/spring-boot-h2-console-error", "baeldung h2 error", now, now.plusDays(2)));
-- createUserLink("john", new CreateLinkRequestDto("?as=u&utm_source=inproduct", "", now.minusDays(2), now.plusDays(2)));
-- createUserLink("john", new CreateLinkRequestDto("","",  now.minusDays(5), now.minusDays(2)));

insert into AUTHENTICATED_USER.LINK (short_link_id, original_url, created_at, updated_at, valid_from, valid_until, description, marked_for_deletion, is_latest)
...
insert into AUTHENTICATED_USER.LINK_USER (link_id, user_name)
